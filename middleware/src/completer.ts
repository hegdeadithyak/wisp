      ...
      llamaRes.on("data", (chunk: string) => {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            const token: string = (json.content ?? "").replace(/[\x00-\x08\x0B-\x1F\x7F]/g, "");
            const stop: boolean = json.stop ?? false;
...
export async function completeOnce(opts: Omit<CompletionOptions, "signal">): Promise<string> {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify({
      prompt: opts.prompt,
      n_predict: opts.maxTokens ?? 48,
    ...
    const req = http.request(
      { hostname: LLAMA_HOST, port: LLAMA_PORT, path: "/completion", method: "POST",
        headers: { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(body) } },
      (res) => {
        let data = "";
        res.on("data", (c) => (data += c));
        res.on("end", () => {
          try { resolve((JSON.parse(data).content ?? "").trim()); }
